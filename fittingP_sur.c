...
void fittingP_sur(){
   ...
   double N_a[N-2] = {0.};
   double err[N-2] = {0.};
   char filename[50] = "survivability-L50-p0.64.txt";
   ...
   auto c0 = new TCanvas("c1","average P_{sur} at critical percolation probability over 10,000 runs",200,10,700,500);
   gr1->SetTitle("average P_{sur} at critical percolation probability over 10,000 runs");
   gr1->GetXaxis()->SetTitle("time - t [mcs]");
   gr1->GetYaxis()->SetTitle("Survival Probability - <P_{sur}>");
   gr1->Draw("AP");
   c0->Update();
}
